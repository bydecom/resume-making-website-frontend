...
const EducationStep = ({ data, updateData, nextStep, prevStep, formData }) => {
  // Extract education data from the most appropriate source
  const extractEducation = () => {
    // If data is already an array with education objects, use it directly
    if (Array.isArray(data) && data.length > 0) {
      return data;
    }
    
    // If formData has education, use that
    if (formData?.education?.length > 0) {
      return formData.education;
    }
    
    // If formData has originalCV with education, use that
    if (formData?.originalCV?.education?.length > 0) {
      return formData.originalCV.education;
    }
  ...
  const [educations, setEducations] = useState(extractEducation() || []);
  ...
  useEffect(() => {
    // Update educations when data or formData changes
    const extractedEducation = extractEducation();
    if (extractedEducation && Array.isArray(extractedEducation)) {
      console.log('Education data loaded:', extractedEducation);
      setEducations(extractedEducation);
    }
  }, [data, formData]);
